...
namespace Report_PricingRange.Business
{
    public static class SqlMapperUtil
    {
        ...
        public static List<T> StoredProcNOParams<T>(string procname, string conn = null)
        {
            using (SqlConnection connection = GetOpenConnection(conn))
            {
                return connection.Query<T>(procname, commandType: CommandType.StoredProcedure, commandTimeout: 0).ToList();
            }
        }
